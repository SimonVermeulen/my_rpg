...
int tick_move_view(object_t *object, engine_t *engine)
{
    ...
    *wait -= get_delta(engine);
    ...
    if (*bloc == 1)
        move_the_view(secondary_screen->bloc_1, destination, engine, start);
    else
        move_the_view(secondary_screen->bloc_2, destination, engine, start);
    if (*start >= length)
        set_active(true, seek_object_scene(object->actual_scene,
        get_value_list(move_view, "enable_object", 4)), engine);
}